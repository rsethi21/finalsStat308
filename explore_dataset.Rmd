...
# Model Selection
```{r}
models <- c(ml, ms, bm, mp, lmp, smp)
nmaxes <- c(length(filtered_data)-1, length(filtered_data)-1, length(filtered_data)-1, length(filtered_data)-1+20, length(filtered_data)-1+20, length(filtered_data)-1+20)
best_model <- none
best_score <- 0

for (i in seq_along(models)) {
  print(i)
  selected_models <- regsubsets(as.formula(models[i]), data=filtered_data, nvmax=nmaxes[i], method="backward")
